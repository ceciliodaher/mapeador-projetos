...
// Verificar senha para seção 9 (ICMS)
...
function goToPreviousStep() {
    if (currentStep > 1) {
        ...
        // Verificar se precisa de senha para seção 9
        if (currentStep === 9 && !checkAnalystAccess()) {
...
function goToStep(step) {
    if (step >= 1 && step <= totalSteps) {
        // Verificar se é seção 9 e se tem acesso
        if (step === 9 && !checkAnalystAccess()) {
            return;
        }
...
function restoreFormData(data) {
    ...
    if (data.currentStep) {
        // Se for seção 9, verificar senha
        if (data.currentStep === 9 && !checkAnalystAccess()) {
            currentStep = 1;
        } else {
