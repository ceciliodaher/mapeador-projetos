...
class MatrizCardRenderer {
  ...
  renderCostSummary(produto, receitas, insumos) {
    ...
    const margem = produto.precoVenda > 0
      ? (produto.precoVenda - custoTotal) / produto.precoVenda
      : 0;
  ...
  renderInsumoSelector(produtoId) {
    ...
    for (const insumo of insumos) {
      ...
      if (insumo.tipo in insumosPorTipo) {
        insumosPorTipo[insumo.tipo].push(insumo);
      }
    }
    ...
    return `
      <div class="matriz-insumo-selector">
        ...
        <div class="insumo-selector-list">
          ...
          ${insumosPorTipo.MP.length === 0 && insumosPorTipo.EMB.length === 0 && insumosPorTipo.UTIL.length === 0 ? `
  ...
  /**
   * Obter emoji por tipo de insumo
   */
