/* =====================================
   CEI-NAVIGATION.JS
   Sistema de navegação para Formulário CEI (14 seções)
...
let currentStep = 1;
const totalSteps = 14; // CEI tem 14 seções
let formData = {};

// IndexedDB Manager
let ceiDBManager = null;
...
async function initializeDatabase() {
    try {
        ceiDBManager = new CEIIndexedDBManager();
        await ceiDBManager.init();
        console.log('✓ IndexedDB CEI pronto para uso');

        // Migrar dados do localStorage se existirem
        await ceiDBManager.migrateFromLocalStorage();
    } catch (error) {
        console.error('✗ Erro ao inicializar IndexedDB CEI:', error);
...
function goToNextStep() {
    // Navegação livre - não valida antes de avançar
...
async function autoSaveData() {
    ...
    try {
        ...
        await ceiDBManager.saveProject(data);
...
async function checkForSavedData() {
    ...
    try {
        const savedProject = await ceiDBManager.loadProject();
